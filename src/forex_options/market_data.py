...
class MarketDataGenerator:
    ...
    def __init__(self, start_date='2023-01-01', end_date='2024-12-31', 
        ...
        self.daily_dates = pd.date_range(start=self.start_date, end=self.end_date, freq='D')
        self.monthly_dates = pd.date_range(start=self.start_date, end=self.end_date, freq='MS')
    ...
    def generate_eur_tnd_daily(self, volatility=0.08, seed=42):
        ...
        for window in window_sizes:
            ...
            self.eur_tnd_daily[vol_col] = self.eur_tnd_daily['Return'].rolling(window=window).std() * np.sqrt(252)
    ...
    def save_data(self, output_dir='data'):
        ...
        if self.eur_tnd_daily is not None:
            self.eur_tnd_daily.to_csv(os.path.join(output_dir, 'eur_tnd_daily_rates.csv'), index=False)
        
        if self.eur_rates_monthly is not None:
            self.eur_rates_monthly.to_csv(os.path.join(output_dir, 'eur_interest_rates_monthly.csv'), index=False)
        
        if self.tnd_rates_monthly is not None:
            self.tnd_rates_monthly.to_csv(os.path.join(output_dir, 'tnd_interest_rates_monthly.csv'), index=False)
    ...
    def load_data(self, input_dir='data'):
        ...
        -------
        bool
            True if all files were loaded, False otherwise
        """
        try:
            self.eur_tnd_daily = pd.read_csv(os.path.join(input_dir, 'eur_tnd_daily_rates.csv'))
            self.eur_tnd_daily['Date'] = pd.to_datetime(self.eur_tnd_daily['Date'])
            
            self.eur_rates_monthly = pd.read_csv(os.path.join(input_dir, 'eur_interest_rates_monthly.csv'))
            self.eur_rates_monthly['Date'] = pd.to_datetime(self.eur_rates_monthly['Date'])
            
            self.tnd_rates_monthly = pd.read_csv(os.path.join(input_dir, 'tnd_interest_rates_monthly.csv'))
            self.tnd_rates_monthly['Date'] = pd.to_datetime(self.tnd_rates_monthly['Date'])
    ...
    def check_or_generate_data(self, output_dir='data'):
        ...
        eur_rates_path = os.path.join(output_dir, 'eur_interest_rates_monthly.csv')
        tnd_rates_path = os.path.join(output_dir, 'tnd_interest_rates_monthly.csv')
    ...
    def calculate_historical_volatility(self, date, window_size=21):
        ...
        if self.eur_tnd_daily is None:
            raise ValueError("No market data available. Load or generate data first.")
        ...
        if len(data) < window_size:
            raise ValueError(f"Not enough historical data for {window_size}-day volatility calculation.")
    ...
    def get_interest_rates(self, date):
        ...
        if self.eur_rates_monthly is None or self.tnd_rates_monthly is None:
            raise ValueError("No interest rate data available. Load or generate data first.")
        ...
        if not eur_mask.any() or not tnd_mask.any():
            raise ValueError(f"No interest rate data available for or before {date}")
    ...
    def get_spot_rate(self, date):
        ...
        """
        if self.eur_tnd_daily is None:
            raise ValueError("No exchange rate data available. Load or generate data first.")
            
        # Find the closest previous date in data
        mask = self.eur_tnd_daily['Date'] <= date
        
        if not mask.any():
            raise ValueError(f"No exchange rate data available for or before {date}")
    ...
    def get_market_data(self, date, volatility_window=21):
        ...
        try:
            ...
            volatility = self.calculate_historical_volatility(date, volatility_window)
    ...
    def plot_market_data(self, output_dir='plots'):
        ...
        if self.eur_tnd_daily is not None:
            ...
            for window in [5, 21, 63]:
                vol_col = f'{window}d_Volatility'
                plt.plot(self.eur_tnd_daily['Date'], self.eur_tnd_daily[vol_col], label=f'{window}-day volatility')
        ...
        if self.eur_rates_monthly is not None and self.tnd_rates_monthly is not None:
            # Plot interest rates
            plt.figure(figsize=(12, 6))
            plt.plot(self.eur_rates_monthly['Date'], self.eur_rates_monthly['EUR_Rate'], label='EUR Interest Rate')
            plt.plot(self.tnd_rates_monthly['Date'], self.tnd_rates_monthly['TND_Rate'], label='TND Interest Rate')
            ...
            differential = self.tnd_rates_monthly['TND_Rate'].values - self.eur_rates_monthly['EUR_Rate'].values
            ...
            plt.grid(True)
            plt.savefig(os.path.join(output_dir, 'interest_rate_differential.png'))
            plt.close()