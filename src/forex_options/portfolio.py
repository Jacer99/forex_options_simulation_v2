...
class PortfolioManager:
    ...
    def price_portfolio(self, pricing_date, models=None, model_params=None):
        ...
        if portfolio is None or portfolio.empty:
            raise ValueError("No options portfolio available")
        ...
        for _, option in portfolio.iterrows():
            ...
            for model in models:
                try:
                    ...
                    # Price the option
                    pricing_result = self.pricing_engine.price_option(model, params)
                    
                    # Calculate option value in EUR
                    notional = option['NotionalEUR']
                    contract_size = notional / option['SpotRate']  # Number of TND units
                    ...
                    print(f"Error pricing option {option['OptionID']} with model {model}: {e}")
    ...
    def calculate_portfolio_risk(self, results=None):
        ...
        -------
        dict
            Dictionary with portfolio risk metrics by model
        """
        if results is None:
            results = self.portfolio_results
        
        if results is None or results.empty:
            raise ValueError("No portfolio results available. Run price_portfolio first.")
        ...
        for model in results['model'].unique():
            ...
            # Aggregate Greeks
            total_delta = (model_results['delta'] * model_results['notional_eur']).sum()
            
            # Optional Greeks (may not be available for all models)
            total_gamma = None
            if 'gamma' in model_results and not model_results['gamma'].isna().all():
                total_gamma = (model_results['gamma'] * model_results['notional_eur']).sum()
                
            total_vega = None
            if 'vega' in model_results and not model_results['vega'].isna().all():
                total_vega = (model_results['vega'] * model_results['notional_eur']).sum()
                
            total_theta = None
            if 'theta' in model_results and not model_results['theta'].isna().all():
                total_theta = (model_results['theta'] * model_results['notional_eur']).sum()
                
            total_rho_d = None
            if 'rho_d' in model_results and not model_results['rho_d'].isna().all():
                total_rho_d = (model_results['rho_d'] * model_results['notional_eur']).sum()
                
            total_rho_f = None
            if 'rho_f' in model_results and not model_results['rho_f'].isna().all():
                total_rho_f = (model_results['rho_f'] * model_results['notional_eur']).sum()
    ...
    def calculate_exposure_by_maturity(self, results=None):
        ...
        for model in results['model'].unique():
            ...
            for i in range(len(buckets)):
                if i < len(buckets) - 1:
                    mask = (model_results['T_months'] >= buckets[i]) & (model_results['T_months'] < buckets[i+1])
                else:
                    mask = model_results['T_months'] >= buckets[i]
                
                bucket_exposure[bucket_labels[i]] = model_results.loc[mask, 'option_value_eur'].sum()
    ...
    def calculate_exposure_by_moneyness(self, results=None):
        ...
        for model in results['model'].unique():
            ...
            model_results['moneyness'] = model_results['strike'] / model_results['spot']
            ...
            for i in range(len(buckets) - 1):
                mask = (model_results['moneyness'] >= buckets[i]) & (model_results['moneyness'] < buckets[i+1])
                bucket_exposure[bucket_labels[i]] = model_results.loc[mask, 'option_value_eur'].sum()
    ...
    def calculate_pnl(self, results_t1, results_t0=None):
        ...
        common_options = set(results_t0['option_id']) & set(results_t1['option_id'])
        ...
        for model in results_t0['model'].unique():
            ...
            for option_id in common_options:
                ...
                delta_pnl = t0_delta * (t1_spot - t0_spot) * opt_t0['notional_eur'].values[0] / t0_spot
    ...
    def calculate_model_comparison(self, reference_model='black_scholes'):
        ...
        ref_results = self.portfolio_results[self.portfolio_results['model'] == reference_model]
            ...
            if 'gamma' in model_results and 'gamma' in ref_results:
                for opt in common_options:
                    if pd.notna(model_results.loc[opt, 'gamma']) and pd.notna(ref_results.loc[opt, 'gamma']):
                        gamma_diff.append(model_results.loc[opt, 'gamma'] - ref_results.loc[opt, 'gamma'])
            ...
            if 'vega' in model_results and 'vega' in ref_results:
                for opt in common_options:
                    if pd.notna(model_results.loc[opt, 'vega']) and pd.notna(ref_results.loc[opt, 'vega']):
                        vega_diff.append(model_results.loc[opt, 'vega'] - ref_results.loc[opt, 'vega'])
    ...
    def save_pricing_results(self, output_dir='results'):
        ...
        if self.portfolio_results is not None and not self.portfolio_results.empty:
            ...
            self.portfolio_results.to_csv(os.path.join(output_dir, 'portfolio_pricing_results.csv'), index=False)
            
            # Save results by model
            for model in self.portfolio_results['model'].unique():
                model_results = self.portfolio_results[self.portfolio_results['model'] == model]
                model_results.to_csv(os.path.join(output_dir, f'{model}_pricing_results.csv'), index=False)
    ...
    def load_pricing_results(self, input_dir='results'):
        ...
        """
        try:
            results_path = os.path.join(input_dir, 'portfolio_pricing_results.csv')
            
            if os.path.exists(results_path):
                self.portfolio_results = pd.read_csv(results_path)
                self.portfolio_results['pricing_date'] = pd.to_datetime(self.portfolio_results['pricing_date'])
                self.portfolio_results['issue_date'] = pd.to_datetime(self.portfolio_results['issue_date'])
                self.portfolio_results['maturity_date'] = pd.to_datetime(self.portfolio_results['maturity_date'])
                return True
            else:
                return False
        except Exception as e:
