...
class OptionsGenerator:
    ...
    def generate_options_portfolio(self, n_options=30, seed=42):
        ...
        if year_data.empty:
            raise ValueError(f"No market data available for the simulation year {self.simulation_year}")
        ...
        while len(options) < n_options and total_notional < self.max_notional:
            ...
            issue_date = np.random.choice(available_dates[:-30])  # Ensure some time before year end
            
            # Determine maturity (between 5 days and 1 year)
            min_days = 5
            max_days = min(360, (pd.Timestamp(f"{self.simulation_year}-12-31") - issue_date).days)
            ...
            spot_rate = year_data[year_data['Date'] == issue_date]['EUR/TND'].values[0]
            
            # Generate strike price with skew (more OTM calls)
            strike_skew = np.random.beta(2, 5)  # Beta distribution for asymmetric strikes
            strike_percent = 0.9 + 0.2 * strike_skew  # Strikes between 90% and 110% of spot
    ...
    def save_options(self, output_dir='data'):
        ...
        if self.options_portfolio is not None:
            self.options_portfolio.to_csv(os.path.join(output_dir, 'options_portfolio.csv'), index=False)
    
    def load_options(self, input_dir='data'):
        """
        Load options portfolio from a CSV file.
        ...
        """
        try:
            self.options_portfolio = pd.read_csv(os.path.join(input_dir, 'options_portfolio.csv'))
            self.options_portfolio['IssueDate'] = pd.to_datetime(self.options_portfolio['IssueDate'])
            self.options_portfolio['MaturityDate'] = pd.to_datetime(self.options_portfolio['MaturityDate'])
            return True
    ...
    def plot_options_distribution(self, output_dir='plots'):
        ...
        if self.options_portfolio is None:
            raise ValueError("No options portfolio available. Generate or load options first.")
